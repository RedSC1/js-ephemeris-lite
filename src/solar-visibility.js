...
function gmstRadians(jdUT1, jdTT) {
...
function gastRadians(jdUT1, jdTT, nutation) {
  return normalizeRadians(gmstRadians(jdUT1, jdTT)
    + nutation.dpsi * Math.cos(nutation.trueObliquity));
}

function apparentSunEquatorial(jdTT) {
...
function rawSolarSample(jdUT1, observer, options) {
  ...
  const gmst = gmstRadians(jdUT1, jdTT);
  const gast = gastRadians(jdUT1, jdTT, sun.nutation);
