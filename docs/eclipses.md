...
| 阶段 | 寿星接口 | 内容 | 状态 |
| --- | --- | --- | --- |
| 1 | `ecFast` | 快速日食搜索与类型预判 | 已完成 |
| 2 | `ysPL.lineT`、`lecXY`、`lecMax` | 月食接触时刻、食分与类型 | 已完成 |
| 3 | `rsGS.init`、`chazhi`、`sun`、`moon`、`bse` | 日月根数表和贝塞尔坐标基础 | 已完成 |
| 4 | `rsGS.feature`、`qrd`、`jieX`、`jieX2`、`jieX3` | 全球日食特征、食带与界线 | 已完成 |
| 5 | `rsPL.secXY`、`lineT`、`secMax` | 指定地点的日食过程与可见性 | 已完成 |
| 6 | `rsPL.zb0`、`zbXY`、`p2p`、`pp0`、`nbj` | 地方中心及南北界 | 已完成 |
...
月食接触、地方接触以及完整食带界线；后续还会继续增加无中心食、类型分界、
日出带食和日没带食样例。`ecFast` 在 `ac=0` 时已自动调用 `rsGS` 复核；需要
