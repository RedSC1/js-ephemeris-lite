...
export function coneDiscriminantFast(e,radius){
 ...
 for(let i=0;i<24;i++){const f=value(...scan[i]);if(f>best){best=f;at=i*step;}}
 let lo=at-step,hi=at+step,g=(Math.sqrt(5)-1)/2;
 let a=hi-g*(hi-lo),b=lo+g*(hi-lo),fa=evaluate(a),fb=evaluate(b);
 for(let i=0;i<28;i++){
  if(fa>fb){hi=b;b=a;fb=fa;a=hi-g*(hi-lo);fa=evaluate(a);}
  else{lo=a;a=b;fa=fb;b=lo+g*(hi-lo);fb=evaluate(b);}
 }
