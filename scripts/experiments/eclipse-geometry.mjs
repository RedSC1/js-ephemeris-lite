// Experimental C++-style circular-limb geometry on the JS ephemeris.
import { apparentBodyPosition } from '../../src/apparent.js';
import { earthHeliocentricState, moonGeocentricState, AU_KM } from '../../src/ephemeris.js';
...
export function passes(k,lunar=false){
 ...
 return Math.abs(Math.sin(F))<=Math.sin(23*D);
}
export function seed(k,lunar=false){
 ...
 if(!lunar)return t-.4075*Math.sin(p)+.1721*e*Math.sin(m);
 ...
 if(f<0)f+=180;f=(f-.02665*Math.sin(omega))*D;
...
export function pairProvider(center,window=false){
 const counts={exactPairs:0,geometricPairs:0};
 const raw=t=>{counts.geometricPairs++;const jd=2451545+t;return [moonGeocentricState(jd).position,scale(earthHeliocentricState(jd).position,-AU_KM)];};
 const exact=(t,frame='true-of-date')=>{counts.exactPairs++;return ['moon','sun'].map(body=>{
  const a=apparentBodyPosition(body,2451545+t,{solarDeflection:false,frame});
  return scale(frame==='j2000'?a.eclipticPositionAu:a.equatorialPositionAu,AU_KM);
 });};
 const cache=new Map(),step=.125;
 function sample(i){if(!cache.has(i)){const t=center+i*step,g=raw(t),a=exact(t,'j2000');cache.set(i,a.map((v,b)=>sub(v,g[b])));}return cache.get(i);}
 if(window)for(let i=-4;i<=4;i++)sample(i);
 const evaluate=t=>{
  if(!window)return exact(t);
  const x=(t-center)/step,i=Math.floor(x),f=x-i,a=sample(i),b=sample(i+1),g=raw(t);
  const jd=2451545+t,n=iau2000bNutation(jd),m=meanEclipticOfDateMatrixState(jd).matrix;
  return g.map((v,k)=>{
   const p=add(v,a[k].map((z,j)=>z*(1-f)+b[k][j]*f));
   const q=[dot(m[0],p),dot(m[1],p),dot(m[2],p)];
   const c=Math.cos(n.dpsi),s=Math.sin(n.dpsi);
   return rx([c*q[0]-s*q[1],s*q[0]+c*q[1],q[2]],n.trueObliquity);
  });
 };
...
export function coneDiscriminant(e,radius){
 function evaluate(t){
  ...
  const a=lineIntersection(o,v,Math.PI/2+e.d);return a.vertex>0?a.disc:-Infinity;
 }
 const step=Math.PI/12;let best=-Infinity,at=0;
 for(let i=0;i<24;i++){const f=evaluate(i*step);if(f>best){best=f;at=i*step;}}
 let lo=at-step,hi=at+step;const g=(Math.sqrt(5)-1)/2;
 let c=hi-g*(hi-lo),d=lo+g*(hi-lo),fc=evaluate(c),fd=evaluate(d);
 for(let i=0;i<28;i++){
  if(fc>fd){hi=d;d=c;fd=fc;c=hi-g*(hi-lo);fc=evaluate(c);}
  else{lo=c;c=d;fc=fd;d=lo+g*(hi-lo);fd=evaluate(d);}
 }
 return Math.max(fc,fd);
}
export function bisect(fn,lo,hi,tolerance=1e-10){
 let fl=fn(lo),fh=fn(hi);if(fl*fh>0)throw Error('unbracketed contact');
 for(let i=0;i<60&&hi-lo>tolerance;i++){const m=(lo+hi)/2,f=fn(m);if(fl*f<=0){hi=m;fh=f;}else{lo=m;fl=f;}}
 return (lo+hi)/2;
}
export function directContact(fn,t,side){
 let inner=t,fi=fn(t),outer=null,fo;
 if(fi<0)return null;
 for(const h of [.5,1,2,4,8,12]){const x=t+side*h/24,f=fn(x);if(f<=0){outer=x;fo=f;break;}inner=x;fi=f;}
 if(outer===null)throw Error('contact outside 12-hour bracket');
 let lo=side<0?outer:inner,hi=side<0?inner:outer,fl=side<0?fo:fi,fh=side<0?fi:fo,last=0;
 for(let i=0;i<40&&hi-lo>.02/86400;i++){
  let fraction=-fl/(fh-fl);if(!(fraction>.05&&fraction<.95))fraction=.5;
  const m=lo+fraction*(hi-lo),f=fn(m);
  if(fl*f<=0){hi=m;fh=f;if(last===1)fl*=.5;last=1;}else{lo=m;fl=f;if(last===-1)fh*=.5;last=-1;}
 }
 return (lo+hi)/2;
}
export function solveSolar(k,{window=false,moonRadius=RM,sunRadius=RS,filter=true,seedTime,providerFactory,greatestSteps,cone=coneDiscriminant}={}){
 if(filter&&!passes(k))return null;
 let t=seedTime??seed(k);const provider=providerFactory?providerFactory(t):pairProvider(t,window),at=t=>solarElements(provider.evaluate(t),moonRadius,sunRadius);
 ...
 for(const h of greatestSteps??[.5,.125,.03125,...[1,.5,.25,.125,.0625,.03125].map(x=>x/1440)]){
  ...
  if(Math.abs(c)>1e-12)t+=Math.max(-h,Math.min(h,.5*(fm-fp)/c*h));
 }
 ...
 let magnitude=null;
 if(isCentral){
  contacts.centralBegin=directContact(central,t,-1);contacts.centralEnd=directContact(central,t,1);
  const signs=[];
  for(const time of [t,contacts.centralBegin+10/86400,contacts.centralEnd-10/86400]){
   const g=at(time),hit=axisIntersection(g);if(!hit.point)continue;
   // Axis distance from Moon to visible Earth surface, in Earth radii.
   const depth=hit.point[2];signs.push(g.l2+(moonRadius/RE-g.l2)/g.z*depth);
   if(time===t){const observer=add(add(scale(g.xh,-hit.point[0]*RE),scale(g.yh,hit.point[1]*RE)),scale(g.axis,-depth*RE));
    magnitude=Math.asin(moonRadius/norm(sub(g.moon,observer)))/Math.asin(sunRadius/norm(sub(g.sun,observer)));}
  }
  if(signs.some(x=>x>0)&&signs.some(x=>x<0))kind='hybrid';else if(signs.length)kind=signs[0]>0?'total':'annular';
 }
 return {kind,maximum:t,contacts,magnitude,counts:provider.counts};
}
