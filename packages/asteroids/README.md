...
`asteroid-ephemeris-lite` 是 `js-ephemeris-lite` 的可选小天体星历包，提供九个小天体在固定 J2000 平黄道坐标轴中的日心和地心几何位置。接口返回三维 AU 向量，可用于一般几何计算，并非只保留黄经或专门面向占星用途。
...
小行星系数不包含在 `js-ephemeris-lite` 主包中，只有安装本包的项目才会下载这些数据。
