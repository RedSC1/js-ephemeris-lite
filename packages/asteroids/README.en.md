...
`asteroid-ephemeris-lite` is the optional minor-body ephemeris package for `js-ephemeris-lite`. It provides heliocentric and geocentric geometric positions for nine minor bodies in fixed mean-J2000 ecliptic axes. The functions return three-dimensional AU vectors for general geometric use; the models are not longitude-only or astrology-specific.
...
The asteroid coefficients are not part of the `js-ephemeris-lite` core package. Projects download them only when they install this optional package.
