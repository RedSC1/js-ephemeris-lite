...
test('orbital events agree with C++ DE441 fixtures at documented model tolerances',()=>{
  ...
  for(const c of fixture.samples) {
    ...
    const found=cache.get(key).find(e=>Math.abs(e.time.jdTT-c.event.jdTT)<0.01);
    assert.ok(found,`${key}: missing event`);
    assert.ok(found.time instanceof JulianTime);
    assert.equal('jdTT' in found, false);
    // Folded Venus measures 3.44 s at greatest elongation and 8.02 s at
    // RA stations in these controls; retain a 10 s envelope for those samples.
    const seconds=c.body==='venus' && ['searchGreatestElongations','searchRightAscensionStations'].includes(c.fn)
      ? 10 : tolerances[c.fn];
    assert.ok(Math.abs(found.time.jdTT-c.oracleJdTT)*86400<seconds,`${c.fn}/${c.body}: model tolerance ${seconds}s`);
  }
...
test('apsides are radial-velocity roots with the advertised physical center',()=>{
  for(const [fn,state] of [[events.searchLunarApsides,moonGeocentricState],[events.searchEarthApsides,earthHeliocentricState]]) {
    for(const e of fn(2461041.5,2461406.5)) {
      ...
      const center=radius(e.time.jdTT),before=radius(e.time.jdTT-0.05),after=radius(e.time.jdTT+0.05);
...
test('lunar node planes remain distinct; true equinox only shifts the longitude',()=>{
  ...
  for(let i=0;i<mean.length;i++) {
    ...
    assert.equal(mean[i].time.jdTT,truth[i].time.jdTT);
    assert.notEqual(mean[i].longitudeDeg,truth[i].longitudeDeg);
    assert.ok(Math.abs(mean[i].time.jdTT-fixed[i].time.jdTT)>1e-7);
  }
});
...
test('greatest elongations are 3D maxima, not conjunction minima',()=>{
  ...
  for(const r of e) for(const offset of [-0.05,0.05]) assert.ok(r.elongationDeg>bodyPhenomena('mercury',r.time.jdTT+offset).solarElongationDeg);
});
...
test('right ascension events are not accidentally ecliptic-longitude events',()=>{
  ...
  assert.ok(Math.abs(apparentBodyState('mercury',station.time.jdTT).longitudeSpeedDegPerDay)>1e-4);
  const roots=events.searchRelativeRightAscension('venus','moon',0,2461041.5,2461072.5);
  for(const r of roots) {
    const a=apparentBodyPosition('venus',r.time.jdTT),b=apparentBodyPosition('moon',r.time.jdTT);
